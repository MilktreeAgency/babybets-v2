-- Bulk reveal all unrevealed instant-win tickets in one transaction (fixes partial reveals from 65+ client round-trips)

CREATE OR REPLACE FUNCTION public.reveal_all_instant_win_tickets(p_user_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
COMMENT ON FUNCTION public.reveal_all_instant_win_tickets(UUID) IS
  'Reveals all unrevealed instant-win tickets for the current user in one transaction, then allocates any winning prizes';

GRANT EXECUTE ON FUNCTION public.reveal_all_instant_win_tickets(UUID) TO authenticated;
