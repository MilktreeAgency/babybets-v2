-- ============================================
-- FIX AUTO EXECUTE DRAWS FUNCTION
-- Description: Fix table name from tickets to ticket_allocations
-- Date: 2026-02-13
-- ============================================
...
-- Recreate with correct table names and columns
...
BEGIN
  ...
  FROM ticket_allocations
  WHERE competition_id = p_competition_id
    AND sold_to_user_id = v_winning_ticket.sold_to_user_id
    AND is_sold = true;
...
COMMENT ON FUNCTION public.execute_competition_draw_internal IS
  'Internal function for executing draws. Uses ticket_allocations table. Bypasses admin check for automated execution.';

-- Recreate the auto execute function
...
COMMENT ON FUNCTION public.auto_execute_competition_draws IS
  'Automatically executes draws for competitions past their draw_datetime. Uses ticket_allocations table.';
