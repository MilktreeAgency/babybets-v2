-- Allow influencer approval updates (when user_id goes from null to value)
-- This enables the approve_influencer_application SECURITY DEFINER function to work
...
COMMENT ON FUNCTION public.protect_influencer_critical_fields IS
  'Prevents non-admin users from modifying critical system-managed fields. Allows: admin users, influencer approvals (user_id null→value + is_active false→true), and SECURITY DEFINER commission functions with specific field patterns.';
