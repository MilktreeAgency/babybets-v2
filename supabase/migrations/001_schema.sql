...
-- Description: Complete database schema (types, tables, indexes)
...
CREATE TABLE public.influencers (
  ...
  -- Social profile URL (from migration 013)
  ...
  -- Unique constraint on user_id (from migration 016)
...
CREATE INDEX idx_influencers_social_profile_url ON public.influencers(social_profile_url);
...
COMMENT ON CONSTRAINT influencers_user_id_key ON public.influencers IS 'Ensures each user can only have one influencer application';
