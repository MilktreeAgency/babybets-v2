-- ============================================
-- BABYBETS DATABASE FUNCTIONS
-- Description: All database functions and stored procedures
...
-- Calculate commission tier based on monthly sales
...
-- Recalculate all commissions for an influencer's current month
...
-- Reset monthly sales (to be called at start of each month)
...
-- ============================================
-- INFLUENCER SALE CREATION (FINAL VERSION)
-- ============================================

-- Create influencer sale and recalculate monthly commissions (NO FRAUD LOGIC)
...
-- ============================================
-- INFLUENCER PROTECTION TRIGGER FUNCTION
-- ============================================

-- Protect critical influencer fields from unauthorized updates
...
BEGIN
  ...
  IF (
    ...
    -- This is a system update (all stats fields being updated together)
    -- Allow it to proceed
    RETURN NEW;
  END IF;
...
COMMENT ON FUNCTION public.protect_influencer_critical_fields IS
  'Prevents non-admin users from modifying critical system-managed fields, but allows SECURITY DEFINER functions to update stats and commission_tier together';
...
-- ============================================
-- TICKET POOL GENERATION FUNCTIONS
-- ============================================
...
-- Generate random unique ticket numbers for a competition
CREATE OR REPLACE FUNCTION public.generate_random_ticket_numbers(
  p_competition_id UUID,
  p_count INTEGER
)
RETURNS TEXT[]
...
AS $$
DECLARE
  v_ticket_numbers TEXT[] := ARRAY[]::TEXT[];
...
COMMENT ON FUNCTION public.generate_random_ticket_numbers(UUID, INTEGER) IS
  'Generates N unique random 7-character alphanumeric ticket numbers for a competition';

-- Generate ticket pool with random alphanumeric codes
