...
export default function Users() {
  ...
  const getRoleBadgeColor = (role: UserRole | null) => {
    const colors: Record<UserRole, string> = {
      user: 'bg-admin-gray-bg text-admin-gray-text',
      influencer: 'bg-admin-purple-bg text-admin-purple-fg',
      admin: 'bg-admin-info-bg text-admin-info-fg',
      super_admin: 'bg-admin-error-bg text-admin-error-text',
    }
    return role ? colors[role] : 'bg-admin-gray-bg text-admin-gray-text'
  }
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-border">
                    {filteredUsers.map((user) => (
                      <tr key={user.id} className="hover:bg-admin-hover-bg cursor-pointer">
                          ...
                          <span
                            className={
                              (user.wallet_balance_pence || 0) > 0
                                ? 'text-admin-success-fg font-medium'
                        ...
                        <td className="px-6 py-4 text-right text-sm">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <Link
                              ...
                              className="inline-flex items-center gap-1 px-3 py-1.5 text-sm text-admin-info-fg hover:text-admin-info-text font-medium cursor-pointer"
                            >
                            ...
                            <Button
                              ...
                              className="text-admin-error-text hover:text-admin-error-light hover:bg-admin-error-bg cursor-pointer"
                            >
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            {/* User Info */}
            {userToDelete && (
              <div className="flex items-center gap-3 p-4 bg-admin-error-bg border border-admin-error-border rounded-lg">
            ...
            {/* Warning */}
            <div className="p-4 bg-admin-warning-bg border border-admin-warning-fg rounded-lg">
              <p className="text-sm text-admin-warning-fg">
                <strong>Warning:</strong> Deleting this user will also delete:
              </p>
              <ul className="mt-2 text-sm text-admin-warning-fg list-disc list-inside space-y-1">
